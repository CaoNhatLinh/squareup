...
export default function ItemLibrary() {
  ...
  useEffect(() => {
    const loadItems = async () => {
      setLoading(true);
      try {
        const data = await fetchItems(restaurantId, {
          page,
          limit,
          q: searchQuery,
        });
    ...
    loadItems();
  }, [restaurantId, page, limit, searchQuery, loaderData, refreshKey]);
  ...
  const refetchItems = () => setRefreshKey((prev) => prev + 1);
      ...
      <PageHeader
        ...
        rightChildren={
          <div className="flex items-center gap-2">
            ...
            >
              <HiOutlinePlusCircle className="w-4 h-4" /> Quick Create
            </button>
          </div>
        }
      />
        ...
        <Table
          columns={[
            ...
            },
            {
              key: "price",
              title: "Price",
              render: (r) => `$${r.price?.toFixed(2) || "0.00"}`,
            },
            {
              key: "actions",
              title: "Actions",
              render: (r) => (
                <div className="flex justify-end">
                  <ActionMenu
                    isOpen={itemMenus[r.id]}
                    onToggle={(open) =>
                      setItemMenus({ ...itemMenus, [r.id]: open })
                    }
                    editPath={`/restaurant/items/${r.id}/edit`}
