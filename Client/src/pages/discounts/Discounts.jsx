...
export default function Discounts() {
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      await Promise.all(
        selectedDiscounts.map((id) => deleteDiscount(restaurantId, id))
      );
        ...
        <div className="p-4">
          <Table
                ...
                title: (
                  <input
                    ...
                    onChange={(e) => {
                      if (e.target.checked)
                        setSelectedDiscounts(
                          (discounts || []).map((c) => c.id)
                        );
                      else setSelectedDiscounts([]);
                    }}
                  />
                ),
                render: (r) => (
                  <Checkbox
                    checked={selectedDiscounts.includes(r.id)}
                    onChange={(e) => {
                      e.stopPropagation();
                      handleSelectDiscount(r.id);
                    }}
                  />
                ),
              },
              ...
              },
              {
                ...
                render: (r) => (
                  <div className="flex justify-end">
                    <ActionMenu
                      isOpen={discountMenus[r.id]}
                      onToggle={(open) =>
                        setDiscountMenus({ ...discountMenus, [r.id]: open })
                      }
                      editPath={`/restaurant/discounts/${r.id}/edit`}
            ...
            onPageChange={(p) => {
              setSelectedDiscounts([]);
              setPage(p);
              loadDiscounts({ page: p, limit });
            }}
            onLimitChange={(l) => {
              setSelectedDiscounts([]);
              setLimit(l);
              setPage(1);
              loadDiscounts({ page: 1, limit: l });
            }}
          />
        </div>
      )}
