...
export default function RestaurantInfoDrawer({ isOpen, onClose, restaurant }) {
  ...
  const formatBusinessHours = (hours) => {
    ...
    return DAYS_OF_WEEK.map(day => {
      ...
      const dayLabelObj = DAY_LABELS.find(d => d.key === day);
      const dayLabel = dayLabelObj ? dayLabelObj.label : day;

      if (!dayData) return { day: dayLabel, hours: 'Not set', isClosed: false };
      return {
        day: dayLabel,
  ...
  return (
    <>
      ...
      />
      <div
        className={`fixed left-0 top-0 h-full w-full max-w-md bg-white shadow-2xl transform transition-transform duration-300 z-50 overflow-y-auto ${isOpen ? 'translate-x-0' : '-translate-x-full'
          }`}
      >
            ...
            <div className="space-y-3">
              ...
              <div className="space-y-2">
                {hoursData.map((dayInfo, idx) => (
                  <div
                    key={idx}
                    className={`flex justify-between items-center py-2 px-3 rounded-lg ${dayInfo.day === today
                        ? 'bg-red-50 border border-red-200'
                        : 'bg-gray-50'
                      }`}
                  >
                    <span className={`font-medium ${dayInfo.day === today ? 'text-red-900' : 'text-gray-700'
                      }`}>
                      {dayInfo.day}
                    </span>
                    <span className={`${dayInfo.isClosed
