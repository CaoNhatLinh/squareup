...
                  <div>
                        ...
                        {(() => {
                            const itemDiscountData = order.discount?.itemDiscounts?.[item.groupKey];
                            const isItemDiscounted = itemDiscountData?.discountAmount > 0;
                            const discountName = order.discount?.appliedDiscounts?.[0]?.name;
                            ...
                            return (
                                <div className="space-y-1">
                                    <h3 className="text-lg font-bold text-gray-900 flex items-center gap-2">
                                        <span>{item.name}</span>
                                        <span 
                                            title={`Applied Discount: ${discountName || 'BOGO Rule'}`}
                                            className="inline-flex items-center gap-1 px-2 py-0.5 bg-yellow-100 text-yellow-800 rounded-full text-xs font-semibold cursor-help"
                                        >
                                    ...
                                    <p className="text-sm text-gray-500 line-through">
                                        Original Price: ${Number(itemDiscountData.originalPrice || 0).toFixed(2)}
                                    </p>
                                </div>
                            );
                        })()}
